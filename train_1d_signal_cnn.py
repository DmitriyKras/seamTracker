...
import os
import time
from utils import SeamDataset1DSignal, SeamRegressor1D


# , linear, 0.0410, RMSE Loss
...
def build_dss(data_dir: str) -> Tuple[ConcatDataset, ConcatDataset]:
    ...
    train = ConcatDataset([
        SeamDataset1DSignal(d) for d in train
    ])
    val = ConcatDataset([
        SeamDataset1DSignal(d) for d in val
    ])
    return train, val
